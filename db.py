"""
Database utilities for the Booking API.
...
def init_db(conn: sqlite3.Connection) -> None:
    ...
    classes = [
        ("Yoga", (now + timedelta(days=1, hours=7)).strftime('%Y-%m-%d %H:%M:%S'), "Alice", 3),
        ("Zumba", (now + timedelta(days=2, hours=9)).strftime('%Y-%m-%d %H:%M:%S'), "Bob", 6),
